                            ...
                            each(rules,
                                function(cssRule) {
                                    if (cssRule.styleSheet) {
                                        ...
                                        each(cssRule.selectorText.split(","),
                                            function(selector) {
...
// Exclusive mode can only be disabled when there are groups allowing the same style to be present in multiple groups
                    ...
                    editor.on("renderFormatsMenu",
                        function(e) {
                            ...
                            each(getSelectors(e.doc || editor.getDoc(), compileFilter(settings.importcss_file_filter)),
                                function(selector) {
                                    if (selector.indexOf(".mce-") === -1) {
                                        if (!selectorFilter || selectorFilter(selector)) {
                                            var selectorGroups = getGroupsBySelector(groups, selector);
                                            if (selectorGroups.length > 0) {
                                                Tools.each(selectorGroups,
                                                    function(group) {
