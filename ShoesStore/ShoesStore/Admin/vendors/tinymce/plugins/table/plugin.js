        ...
        function(Tools, Env, Utils, SplitCols) {
            ...
            return function(editor, table, selectedCell) {
                ...
                function getChildrenByName(node, names) {
                    ...
                    names = Tools.map(names.split(","),
                        function(name) {
                            return name.toLowerCase();
                        });
                    return Tools.grep(node.childNodes,
                        function(node) {
                    ...
                    each(["thead", "tbody", "tfoot"],
                        function(part) {
                            ...
                            each(rows,
                                function(tr, y) {
                                    y += startY;
                                    each(getChildrenByName(tr, "td,th"),
                                        function(td, x) {
                ...
                function getSelectedRows() {
                    ...
                    each(table.rows,
                        function(row) {
                            each(row.cells,
                                function(cell) {
                                    if (dom.getAttrib(cell, "data-mce-selected") ||
                ...
                function cloneCell(cell) {
                    ...
                    if (editor.settings.table_clone_elements !== false) {
                        cloneFormats = Tools.makeMap(
                            (editor.settings.table_clone_elements || "strong em b i span font h1 h2 h3 h4 h5 h6 p div")
                            .toUpperCase(),
                            /[ ,]/
                        );
                    }
// Clone formats
                    Tools.walk(cell,
                        function(node) {
                            var curNode;
                            if (node.nodeType == 3) {
                                each(dom.getParents(node.parentNode, null, cell).reverse(),
                                    function(node) {
...
// Empty header/body/footer
                    each(dom.select("thead,tbody,tfoot", table),
                        function(part) {
                ...
                function reduceRowSpans(grid, startX, startY, endX, endY) {
                    ...
                    for (var y = startY + 1; y <= endY; y++) {
                        ...
                        if (allCells.length === fakeCells.length) {
                            Tools.each(getUniqueElms(fakeCells),
                                function(elm) {
...
// Try to expand text selection as much as we can only Gecko supports cell selection
                ...
                editor.on("KeyUp Drop SetContent",
                    function(e) {
                        clear(e.type == "setcontent");
                        endSelection();
                        resizing = false;
                    });
                editor.on("ObjectResizeStart ObjectResized",
                    function(e) {
...
// Toggle caption on/off
                    ...
                    if (isProps === true) {
                        ...
                        if (tableElm) {
                            data = {
                                width: removePxSuffix(dom.getStyle(tableElm, "width") ||
                                    dom.getAttrib(tableElm, "width")),
                                height: removePxSuffix(dom.getStyle(tableElm, "height") ||
                                    ...
                                    if (item.value) {
                                        item.textStyle = function() {
                                            return editor.formatter.getCssText(
                                                { block: "table", classes: [item.value] });
                                        };
                                    }
                                }
                            )
                ...
                function drawCols(cellPositions, tableHeight, tablePosition) {
                    Tools.each(cellPositions,
                        function(cellPosition) {
                            ...
                            editor.dom.add(getBody(),
                                "div",
                                generateBar(RESIZE_BAR_COL_CLASS,
...
// Get a matrix of the cells in each row and the rows in the table.
                function getTableDetails(table) {
                    return Tools.map(table.rows,
                        function(row) {
                            var cells = Tools.map(row.cells,
                                function(cell) {
...
// Get a grid model of the table.
                function getTableGrid(tableDetails) {
                    ...
                    var maxRows = 0;
                    var maxCols = 0;
                    Tools.each(tableDetails,
                        function(row, rowIndex) {
                            var currentRow = [];
                            Tools.each(row.cells,
                                function(cell) {
                                    var start = 0;
...
// Adjust the height of the row of table at index, with delta.
                ...
                function drop() {
                    ...
                    if (dragging) {
                        ...
                        if (isCol(dragBar)) {
                            var initialLeft =
                                parseInt(editor.dom.getAttrib(dragBar, RESIZE_BAR_COL_DATA_INITIAL_LEFT_ATTRIBUTE), 10);
                            ...
                            var initialTop =
                                parseInt(editor.dom.getAttrib(dragBar, RESIZE_BAR_ROW_DATA_INITIAL_TOP_ATTRIBUTE), 10);
...
// This fix will force a paragraph element after the table but only when the forced_root_block setting is enabled
                function fixTableCaretPos() {
                    editor.on("KeyDown SetContent VisualAid",
                        function() {
                            var last;
                        ...
                        function(e) {
                            if ((e.keyCode == VK.DELETE || e.keyCode == VK.BACKSPACE) && !e.isDefaultPrevented()) {
                                ...
                                if (table) {
                                    ...
                                    selectedTableCells = Tools.grep(tableCells,
                                        function(cell) {
...
*/
                function handleDeleteInCaption() {
                    ...
                    var isEmptyNode = function(node) {
                        return editor.dom.isEmpty(node) ||
                            node.firstChild === node.lastChild && isCaretContainer(node.firstChild);
                    };
...
// in IE caret is off after restoration
                    ...
                    var contractSelectionTo = function(caption) {
                        ...
                        if (rng.commonAncestorContainer === caption.parentNode &&
                            isTheHeirOf(rng.startContainer, caption)) { // ignore backward selections
...
// over delete operation here to do it manually and restore the placeholder if required
                                ...
                                restoreCaretPlaceholder(container);
                                if (isEmptyNode(container) ||
                                    e.keyCode === VK.BACKSPACE && caretIsAtTheLeftEdgeOf(container)) { // see TINY-979
                                    e.preventDefault();
                                }
                            }
                        });
...
/*jshint validthis:true*/
                ...
                var menuItems = [];
                each("inserttable tableprops deletetable | cell row column".split(" "),
                    function(name) {
                        if (name == "|") {
                            menuItems.push({ text: "-" });
                        } else {
