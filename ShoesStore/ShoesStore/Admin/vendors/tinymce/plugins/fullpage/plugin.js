...
// Parse meta elements
                        each(headerFragment.getAll("meta"),
                            function(meta) {
...
// Parse stylesheets
                        data.stylesheets = [];
                        Tools.each(headerFragment.getAll("link"),
                            function(link) {
...
// Add/update/remove meta
                        each("keywords,description,author,copyright,robots".split(","),
                            function(name) {
...
// Delete old
                        Tools.each(currentStyleSheetsMap,
                            function(stylesheet) {
                                stylesheet.remove();
                            });
...
// Needed for IE 6/7
                        ...
                        var currentStyleSheetsMap = {};
                        Tools.each(headElm.getElementsByTagName("link"),
                            function(stylesheet) {
...
// Delete old
                    ...
                    function getContent(evt) {
                        if (!evt.selection && (!evt.source_view || !editor.getParam("fullpage_hide_in_source_view"))) {
                            evt.content =
                                Protect.unprotectHtml(Tools.trim(head) +
                                    "\n" +
