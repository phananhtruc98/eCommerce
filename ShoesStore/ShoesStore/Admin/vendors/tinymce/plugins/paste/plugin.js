...
// by some additional characters qualifies the text as not a plain text (having some HTML tags)
            ...
            var toBlockElements = function(text, rootTag, rootAttrs) {
                ...
                var paragraphs = Tools.map(blocks,
                    function(p) {
                        return p.split(/\n/).join("<br />");
                    });
...
*/
                function pasteHtml(html, internalFlag) {
                    ...
                    args =
                        editor.fire("BeforePastePreProcess",
                            { content: html, internal: internal }); // Internal event used by Quirks
...
// Create a pastebin
                    pasteBinElm = dom.add(editor.getBody(),
                        "div",
                        {
                            id: "mcepastebin",
...
// If clipboard API has HTML then use that directly
                    ...
                    editor.on("drop",
                        function(e) {
                            ...
                            if ((!hasHtmlOrText(dropContent) || isPlainTextFileUrl(dropContent)) &&
                                pasteImageData(e, rng)) {
                                return;
                            }
                            if (rng && editor.settings.paste_filter_drop !== false) {
                                var content = dropContent["mce-internal"] ||
...
*/
            ...
            function WordFilter(editor) {
                ...
                editor.on("BeforePastePreProcess",
                    function(e) {
                                    ...
                                    [
                                        ...
                                        function(str, spaces) {
                                            return (spaces.length > 0)
                                                ? spaces.replace(/./, " ").slice(Math.floor(spaces.length / 2))
                                                .split("").join("\u00a0")
                                                : "";
                                        }
                                    ]
...
// Produce block regexp based on the block elements in schema
                    var blockElements = [];
                    Tools.each(editor.schema.getBlockElements(),
