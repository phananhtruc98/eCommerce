        ...
        <div class="row">
            <asp:ListView runat="server" OnItemDataBound="lvRcptSub_ItemDataBound" OnPagePropertiesChanging="lvRcptSub_PagePropertiesChanging" AllowPaging="true" ItemType="ShoesStore.DataAccessLogicLayer.RcptSub" OnItemCommand="lvRcptSub_ItemCommand" OnSelectedIndexChanged="lvRcptSub_SelectedIndexChanged" ID="lvRcptSub">
                ...
                <ItemTemplate>
                    <tr>
                        <td>
                            <asp:LinkButton runat="server" ID="lbtnDetail" CommandName="Sel" CommandArgument='<%# Item.RcptSubId %>' CssClass="btn btn-icons btn-rounded btn-outline-warning"><i class="fas fa-info"></i></asp:LinkButton></td>

                        <td>
                            <asp:Label runat="server" ID="lbRcptId" Text="<%# Item.RcptSubId %>"></asp:Label></td>
                        <td>
                            <asp:Label runat="server" ID="lbDateAdd" Text="<%# Item.Rcpt.DateAdd %>"></asp:Label></td>
                        <td>
                            <asp:Label runat="server" ID="lbShpName" Text="<%# Item.Mer.Shp.Select(x=>x.ShpName).FirstOrDefault() %>"></asp:Label></td>
                        <td>
                            <asp:Label runat="server" ID="lbCusName" Text="<%# Item.RcptSubDet.Select(x=>x.Sub.DurDay).Sum() %>"></asp:Label></td>
                        <td>
                            <asp:Label runat="server" ID="Label1" Text="<%# MyLibrary.SumRcptSubPrice(Item) %>"></asp:Label></td>
                    </tr>
                </ItemTemplate>
            </asp:ListView>
