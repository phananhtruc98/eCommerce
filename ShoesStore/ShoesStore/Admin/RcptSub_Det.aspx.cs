...
{
    public partial class RcptSub_Det : Page
    {
        public void LoadLv(int RcptSubId)
        {
            ...
            var usr = MyLibrary.RcptSub_BUS.GetAll().Where(x => x.RcptSubId == RcptSubId).Select(x => x.Mer.Usr)
                .FirstOrDefault();
            lbAddressMer.Text = usr.Address;
            lbMerName.Text = usr.UsrName;
            lbPhoneMer.Text = usr.Phone;
            lbEmail.Text = usr.Email;
            var shp = MyLibrary.RcptSub_BUS.GetAll().Where(x => x.RcptSubId == RcptSubId).Select(x => x.Mer.Shp)
                .FirstOrDefault();
            ...
            lbTotalPrice.Text = MyLibrary
                .SumRcptSubPrice(MyLibrary.RcptSub_BUS.GetAll().Where(x => x.RcptSubId == RcptSubId).FirstOrDefault())
                .ToString();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                var RcptSubId = int.Parse(Request.QueryString["RcptSubId"]);
                LoadLv(RcptSubId);
            }
        }
    }
