                ...
                ((HiddenField) x.FindControl("hdfSubId")).Value == ((HiddenField) rs.FindControl("hdfSubId")).Value))
            {
                ...
                var slDaThem = int.Parse(lstSub
                                             .Where(x => int.Parse(((HiddenField) x.FindControl("hdfSubId")).Value) ==
                                                         subId)
                                             .Select(x => ((TextBox) x.FindControl("Qty")).Text).FirstOrDefault() ??
                                         throw new InvalidOperationException());
                var slTong = slThem + slDaThem;
                var firstOrDefault = lstSub
                    .Where(x => int.Parse(((HiddenField) x.FindControl("hdfSubId")).Value) == subId).Select(x =>
                    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (!string.IsNullOrEmpty(payerId))
                {
                    ...
                    {
                        var subQties = ((IEnumerable<SubQty>) Session["listSubs"]).ToList();
